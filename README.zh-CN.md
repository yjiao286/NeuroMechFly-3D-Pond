...
- 起飞去吃的路上会**预约座位**（`inbound`，只统计 36 单位内"马上就到"的），
  判定占用时排除自己，否则会变成"自己占着自己的位、谁都不敢落"的死锁
  （这个坑实测踩过：95% 的时间没有任何一只在吃）；开吃闸门带粘性
  （已在吃的保持、没吃则要求餐位空着），堵死同帧到达的竞态；
...
实测 4 分钟（青蛙静止）：单饵同时进食恒为 1（超员 0.0%）；等位盘旋中位 <1 秒、
最长 2 秒；没有出现 >8 秒的"绕食不吃"僵局。
